...
class Model:
    def __init__(self,coinbase,csv_price,csv_transactions):
        ...
        self.clearance_threshold = 70           # 残币清仓RSI阈值，调高使清仓条件更严格，0~100
        '''在这里调参'''
        '''Hyperparameters'''
    ...
    def calculateRSI(self, period, RRSI=True):
        ...
        if (RRSI) & (length>(self.RSIperiod+self.ma1)):
            ...
            for i in range(self.ma1, 1, -1):
                RRSI.append(self.ma_dataframe.ix[length-1, 'RSI'].item() - self.ma_dataframe.ix[length-i, 'RSI'].item())
            self.ma_dataframe['RRSI'] = np.min(RRSI)
    ...
    def tradeEvaluation(self, balance, quantity):
        ...
        if length > self.ma1:
            ...
            if self.sell_first == 0: # wait / tobuy
                ...
                # Action
                if self.state == 'tobuy':
                    if ((self.level < self.level_threshold) and (last_RSI <= cur_RSI) and (RRSI < self.RRSI_threshold)): # 买！
                ...
                if self.state == 'tosell':
                    if (self.level > self.level_threshold) and (last_RSI >= cur_RSI):
                    ...
                    if (cur_price <= self.loss_limit and RRSI < 0): # 止损！！！
                        ...
                        self.level_threshold = self.level - self.loss_limit * 0.998# 降低市价水平阈值，延缓再买入
                        self.state = 'wait'
                        self.sell_first = 0
                        print("My Godness!!!!!")
                else:
